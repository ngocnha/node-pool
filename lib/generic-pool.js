...
Pool.prototype.destroy = function destroy (obj) {
  ...
  this._availableObjects = this._availableObjects.filter(_filterObjWithTimeout)

  this._inUseObjects = this._inUseObjects.filter(_filterObjInUser)

  this._factory.destroy(obj)

  this._ensureMinimum()

  function _filterObjWithTimeout (objWithTimeout) {
    return (objWithTimeout.obj !== obj)
  }

  function _filterObjInUser (objInUse) {
    return (objInUse !== obj)
  }
}
...
Pool.prototype._dispense = function dispense () {
  ...
  function _asyncWhileCondition () {
    return self._availableObjects.length > 0
  }

  function _asyncValidationIterator (next) {
    self._log('dispense() - reusing obj', 'verbose')
    var objWithTimeout = self._availableObjects[0]

    self._factory.validateAsync(objWithTimeout.obj, _validateCallback)

    function _validateCallback (isValid) {
      if (!isValid) {
  ...
  if (waitingCount > 0) {
    if (this._factory.validateAsync) {
      doWhileAsync(_asyncWhileCondition, _asyncValidationIterator, _topUpResources)
      return
    }
...
Pool.prototype.release = function release (obj) {
  ...
  if (this._availableObjects.some(_someObjWithTimeout)) {
    this._log('release called twice for the same resource: ' + (new Error().stack), 'error')
    return
  }

  function _someObjWithTimeout (objWithTimeout) {
    return (objWithTimeout.obj === obj)
  }
...
Pool.prototype.pooled = function pooled (decorated, priority) {
  ...
  return function () {
    ...
    function _onAcquire (err, client) {
      if (err) {
